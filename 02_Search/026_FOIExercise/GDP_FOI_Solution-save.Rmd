...
```{r scatterplot}
plot(allDF$FOI, allDF$NY.GDP.PCAP.PP.KD)
```
...
```{r correlation}
allDF$GDP <- allDF$NY.GDP.PCAP.PP.KD
cor.test(allDF$FOI, allDF$GDP)
```
...
plot(shufdata$FOI, allDF$GDP, xlab="Shuffled FOI", ylab="GDP")
cor(shufdata$FOI, allDF$GDP)
```
